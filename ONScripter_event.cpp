...
void ONScripter::variableEditMode(SDL_KeyboardEvent* event)
{
    ...
    case SDLK_ESCAPE:
        if (variable_edit_mode == EDIT_SELECT_MODE) {
			...
			#endif
			int len = strlen(DEFAULT_WM_TITLE);
			char *buf_ = new char[len*3+1];
			DirectReader::convertFromSJISToUTF8(buf_, DEFAULT_WM_TITLE);
			SDL_SetWindowTitle(window, buf_);
			delete[] buf_;
#endif
            SDL_Delay(100);
#if !SDL_VERSION_ATLEAST(2, 0, 0)
			...
			#if defined(_MSC_VER) && _MSC_VER <= 1200
			#endif
			len = strlen(wm_title_string);
			buf_ = new char[len*3+1];
			DirectReader::convertFromSJISToUTF8(buf_, wm_title_string);
			SDL_SetWindowTitle(window, buf_);
			delete[] buf_;
#endif
            return;
	...
	#if defined(_MSC_VER) && _MSC_VER <= 1200
	#endif
	int len = strlen(wm_edit_string);
	char *buf_ = new char[len*3+1];
	DirectReader::convertFromSJISToUTF8(buf_, wm_edit_string);
	SDL_SetWindowTitle(window, buf_);
	delete[] buf_;
#endif
}
